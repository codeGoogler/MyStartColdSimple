        ...
        if (data != null && data.length > 0) {
            ...
            for (int i = 0; i < X_AXIS_POINT_COUNT; i++) {
                if (mDataValue[i] != 0) {
                   ...
                   /* LogUtil.i("yyh",lastPointX+"-------1111111111-------" +  lastPointY);*/
...
//        mCurvePaint.setShader(lg);
        ...
        mTopTextPaint.setColor(getResources().getColor(R.color.yello_deep));
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawColor(getResources().getColor(R.color.green_listview));
